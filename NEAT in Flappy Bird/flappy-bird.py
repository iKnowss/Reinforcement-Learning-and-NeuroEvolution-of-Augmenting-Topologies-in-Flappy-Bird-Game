...
def eval_genomes(genomes, config):
    ...
    base = Base(FLOOR) #build the floor
    pipes = [Pipe(700)] 
    global score  
    ...
    nets = [] #create an empty list to store all the training neural networks
    birds = [] #create an empty list to store all the training birds
    ge = [] #create an empty list to store all the training genomes
    ...
    for genome_id, genome in genomes: #for each genome
        ...
        birds.append(Bird(230,350)) #create a bird and append the bird in the list
        ge.append(genome) #append the genome in the list

    run = True
    
    while run and len(birds) > 0: #when we run the program
        ...
        if score > max_score or len(birds) == 0:
...
def run(config_file):
    ...
    node_names = {-1:'bird_location', -2: 'top_pipe', -3:'buttom_pipe', 0:'Jump or Not'}
